...
public class PartialFileDownload implements FileDownload{
    ...
    private ResourceRegion resourceRegion(FileUrlResource fileUrlResource, HttpHeaders httpHeaders) throws IOException {
        final long chunkSize = 1000000L;
        long contentLength = fileUrlResource.contentLength();

        if (httpHeaders.getRange().isEmpty()) {
            return new ResourceRegion(fileUrlResource, 0, Long.min(chunkSize, contentLength));
        }
        ...
        return new ResourceRegion(fileUrlResource, start, rangeLength);
    }
}
